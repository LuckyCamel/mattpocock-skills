---
name: gitlab-triage
description: Triage GitLab issues through a label-based state machine using glab. Use when user wants to create an issue, triage issues, review incoming bugs or feature requests, prepare issues for an AFK agent, or manage issue workflow.
---

# GitLab Issue Triage

Triage issues in the current project using a label-based state machine. Infer the GitLab project from `git remote` (or pass `-R OWNER/REPO` / `GROUP/NAMESPACE/REPO` when needed). Use **`glab`** for all GitLab API/CLI operations (`glab auth` must succeed for your GitLab host).

## GitLab CLI quick reference

Use these patterns (adjust flags as needed):

- **List issues:** `glab issue list` — filter by label: `-l needs-triage` (repeat `-l` for multiple labels); `--all` if needed.
- **View issue:** `glab issue view <iid> --comments` (IID is the per-project issue number shown as `#42`).
- **Create issue:** `glab issue create -t "Title" -d "Description" -y` — add labels with `-l labelname` (repeat per label).
- **Comment (note) on issue:** `glab issue note <iid> -m "body"` (alias: `glab issue comment`).
- **Update labels / title:** `glab issue update <iid> -l newlabel` and `glab issue update <iid> --unlabel oldlabel`.
- **Close issue:** `glab issue close <iid>`.

## AI Disclaimer

Every comment or issue description posted to GitLab during triage **must** include the following disclaimer at the top of the comment body (or at the top of the issue description when creating/updating the main body), before any other content:
...
The maintainer invokes `/gitlab-triage` then describes what they want in natural language. The skill interprets the request and takes the appropriate action.
...
When the maintainer asks for an overview, query GitLab (`glab issue list`, etc.) and present a summary grouped into three buckets:
