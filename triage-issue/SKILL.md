---
name: triage-issue
description: Triage a bug or issue by exploring the codebase to find root cause, then create a GitLab issue with a TDD-based fix plan using glab. Use when user reports a bug, wants to file an issue, mentions "triage", or wants to investigate and plan a fix for a problem.
---
...
Create a GitLab issue using `glab issue create` with the template below (e.g. `-t`, `-d`, `-y`). Do NOT ask the user to review before creating - just create it and share the issue URL.

After creation, print the issue URL and a one-line summary of the root cause.
